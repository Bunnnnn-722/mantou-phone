...
const Prefs = {
  save() {
    ...
    return DB.set('prefs.theme', { theme, palMem, cur: (typeof palCur !== 'undefined' && palCur) || null });
  },
  async restore() {
    ...
    if (p.palMem) {
      ...
      // 旧版曾把豪华装帧写进记忆位,读回来时清掉,昼夜一切仍回经典
      if (typeof SCENE_PALS !== 'undefined') {
        if (SCENE_PALS.has(palMem.dark.pal)) palMem.dark = { id: 'palMo', pal: '' };
        if (SCENE_PALS.has(palMem.light.pal)) palMem.light = { id: 'palZhi', pal: '' };
      }
    }
    setTheme(p.theme || 'dark');
    // 豪华装帧不在 palMem 里,单独恢复
    if (p.cur && p.cur.pal && p.cur.theme === (p.theme || 'dark') && typeof SCENE_PALS !== 'undefined' && SCENE_PALS.has(p.cur.pal)) applyPal(p.cur.id, p.cur.theme, p.cur.pal);
  }
};
